...
void put_object(t_data *data, int type) {
  if (type == 1)
    ...
    mlx_put_image_to_window(data->mlx, data->win,
                            data->img->coin[data->img->current_coin],
                            data->map->x * IMG_W, data->map->y * IMG_H);
    if (data->frame % 10 == 0) {
...
void put_background(t_data *data) {
  ...
  while (y < data->size_y) {
    x = 0;
    while (x < data->size_x) {
      mlx_put_image_to_window(data->mlx, data->win, data->img->background, x,
                              y);
      x += IMG_W;
    }
    y += IMG_W;
...
int ft_render_next_frame(t_data *data) {
  ...
  free(moves_str);
  mlx_string_put(data->mlx, data->win, (data->size_x / 2) - IMG_W,
                 IMG_H / 2 - 10, 0x0000ff, tmp);
  free(tmp);

...
void initialize(t_data *data) {
  ...
  data->img->coin = mlx_xpm_file_to_image(data->mlx, "textures/collectable.xpm",
                                          &width, &height);
  if (!data->img->coin)
    handle_error(data, "Error\ncoin file not found", 1);
  data->img->exit =
      mlx_xpm_file_to_image(data->mlx, "textures/exit.xpm", &width, &height);
  if (!data->img->exit)
